...
@section('content')
    <form action="{{ route('dashboard.mitra.kumbung.update', $kumbung->id) }}" method="POST" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <input 
                                value="{{ old('name') ? old('name') : $kumbung->name }}"
                                type="text" 
                                name="name"
                            ...
                            <select 
                                ...
                                data-width="100%">
                                    @foreach ($budidayas as $budidaya)
                                        <option {{ $kumbung->budidaya->id === $budidaya->id ? 'selected' : '' }} value="{{ $budidaya->id }}">{{ $budidaya->name }}</option>
                                    @endforeach
                            </select>
                            @error('budidaya_id')
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group mb-3">
                                <input 
                                    value="{{ old('name') ? old('name') : $kumbung->large }}"
                                    type="number" 
                                    step="0.01" 
                                    name="large"
                        ...
                        <div class="form-group mb-2">
                            ...
                            <select  name="jamur_id" class="custom-select @error('jamur_id') is-invalid @enderror">
                                ...
                                @foreach ($jamurs as $jamur)
                                    <option {{ $kumbung->jamur->id === $jamur->id ? 'selected' : '' }} value="{{ $jamur->id }}">{{ $jamur->name }}</option>
                                @endforeach
                            </select>
                            @error('jamur_id')
                                <span class="invalid-feedback" role="alert">
            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body">
                        <div id="img-card">
                            <img src="{{ asset('storage/'.$kumbung->photo) }}" class="img-fluid" id="img-budidaya">
                        </div>
                    </div>
                </div>
            </div>
