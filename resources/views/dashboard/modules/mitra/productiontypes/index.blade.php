...
@section('content-header')
  ...
  <div class="row align-items-center">
		<div class="col-md"><h2 class="section-title">Data Produksi Saya</h2></div>
		<div class="col-md-auto">
        	<a href="{{ route('dashboard.mitra.productiontypes.create') }}" class="btn btn-block btn-lg btn-primary"><i class="fas fa-plus mr-2"></i> Tambah Jenis Produksi</a>
        </div>
  </div>
@endsection
    ...
    <div class="card card-primary">
        <div class="card-body">
            ...
            <table class="table">
                ...
                <tbody>
                    @foreach ($productionTypes as $productionType)
                        <tr>
                            <td>{{ $loop->index+1 }}</td>
                            <td width="300px">{{ $productionType->name }}</td>
                            <td >{{ $productionType->description }}</td>
                            <td>
                                <div class="d-flex justify-content-around">
                                    <a href="{{ route('dashboard.mitra.productiontypes.edit', $productionType->id) }}" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i></a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
            ...
            <div class="row justify-content-center justify-content-md-end">
                <div class="col-auto">{{$productionTypes->appends(Request::all())->links()}}</div>
            </div>
        </div>
    </div>
    {{-- end of filter --}}
